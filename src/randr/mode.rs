...
pub(super) fn select_mode(
    ...
    let mode_id = match request {
        ModeRequest::Current => current_mode_id(state, output),
    ...
    let mode = mode_by_id(&state.modes, mode_id).ok_or_else(|| {
...
fn mode_matches(mode: &ModeInfo, width: u16, height: u16, rate: Option<f64>) -> bool {
    mode.width == width
        ...
        && rate.is_none_or(|requested| {
            refresh_millihertz(mode)
                .is_some_and(|actual| actual.abs_diff(requested_millihertz(requested)) <= 500)
        })
}

fn current_mode_id(state: &RandrState, output: &OutputState) -> Option<Mode> {
    if output.crtc == DISABLED_CRTC {
        return None;
    }
    state
...
fn refresh_millihertz(mode: &ModeInfo) -> Option<u32> {
    ...
    let mut refresh = u64::from(mode.dot_clock).checked_mul(1000)? / total;
    if (mode.mode_flags & x11rb::protocol::randr::ModeFlag::INTERLACE)
        == x11rb::protocol::randr::ModeFlag::INTERLACE
    {
        refresh = refresh.checked_mul(2)?;
    }
    if (mode.mode_flags & x11rb::protocol::randr::ModeFlag::DOUBLE_SCAN)
...
fn requested_millihertz(rate: f64) -> u32 {
    (rate * 1000.0).round().clamp(0.0, f64::from(u32::MAX)) as u32
}
...
mod tests {
    ...
    fn mode(id: Mode, width: u16, height: u16, dot_clock: u32) -> ModeInfo {
        mode_with_flags(id, width, height, dot_clock, ModeFlag::from(0_u32))
    }
    ...
    fn state(output: OutputState, modes: Vec<ModeInfo>) -> RandrState {
        RandrState {
            root: 1,
            config_timestamp: 1,
            root_width: 1920,
    ...
    fn selects_preferred_mode_without_intermediate_fallback() {
        ...
        let output = output(vec![11, 12], 1);
        let state = state(output.clone(), modes);
        let selected = select_mode(&state, &output, ModeRequest::Preferred).unwrap();
        assert_eq!(
            selected,
            SelectedMode {
                id: 11,
    ...
    #[test]
    fn selects_explicit_mode_with_interlace_adjusted_refresh_rate() {
        let modes = vec![mode_with_flags(
            11,
            1920,
            1080,
            74_250_000,
    ...
    #[test]
    fn selects_explicit_mode_with_double_scan_adjusted_refresh_rate() {
        let modes = vec![mode_with_flags(
            11,
            1920,
            1080,
            297_000_000,
            ModeFlag::DOUBLE_SCAN,
        )];
        let output = output(vec![11], 1);
        let state = state(output.clone(), modes);
        let selected = select_mode(
    ...
    #[test]
    fn selects_current_active_mode_for_rotation_only_requests() {
        let output = OutputState {
            crtc: 7,
            ..output(vec![11], 1)
        };
        let mut state = state(output.clone(), vec![mode(11, 1920, 1080, 148_500_000)]);
        state.crtcs = vec![CrtcState {
            id: 7,
            x: 0,
            y: 0,
            width: 1920,
        ...
        let selected = select_mode(&state, &output, ModeRequest::Current).unwrap();
